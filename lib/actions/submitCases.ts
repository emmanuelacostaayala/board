...
export async function submitClinicalCases(userId: string, period: string = '2026') {
  try {
    ...
    // 4. Update DB Records (First, so submission is guaranteed)
    const { error: updateError } = await supabaseAdmin
      .from("clinical_case")
      .update({ submission_period: period })
      .in('id', cases.map(c => c.id));

    if (updateError) throw new Error(`Error updating cases status: ${updateError.message}`);

    // 5. Generate Email Content (Simple HTML table)
    ...
    const emailHtml = `
      <div style="font-family: Arial, sans-serif;">
        <h2 style="color: #2563eb;">Submission de Casos Clínicos (${period})</h2>
    ...
    try {
      ...
      await transporter.sendMail({
        ...
        subject: `Entrega de Casos Clínicos ${period} - PCC ${pccCode} - ${userName}`,
        html: emailHtml,
      });
    } catch (emailError) {
      console.error("Error sending submission email (but cases are submitted):", emailError);
      // We proceed to return success because the cases ARE submitted in DB.
    }
